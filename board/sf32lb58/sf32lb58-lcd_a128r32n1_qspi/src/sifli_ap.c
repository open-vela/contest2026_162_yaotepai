...
#include "sf32lb58_lcd_a128r32n1_qspi.h"
#include "bf0_hal.h"
#include "drv_io.h"
 ...
 */
#define SF32LB58_NOR_FS_OFFSET_BLOCKS (0x008A0000 / 4096)
#define SF32LB58_NOR_FS_SIZE_BLOCKS   (0x00400000 / 4096)

#if defined(CONFIG_SPI) && defined(CONFIG_BSP_USING_SPI1)
#  define SF32LB58_SPI1_PORT           0
#endif

#if defined(CONFIG_SPI) && defined(CONFIG_MMCSD_SPI) && \
  defined(CONFIG_BSP_USING_SPI1)
#  define SF32LB58_TFCARD_SPI_PORT     0
#  define SF32LB58_TFCARD_SLOT         0
#  define SF32LB58_TFCARD_MINOR        0
#  define SF32LB58_TFCARD_MOUNTPOINT   "/data/tf"
#endif
...
extern int sf32lb58_bt_initialize(void);
#endif
...
#if defined(CONFIG_SPI) && defined(CONFIG_BSP_USING_SPI1)
static struct spi_dev_s *g_sf32lb58_spi1;

static FAR struct spi_dev_s *sf32lb58_spi1_getbus(void)
{
  if (g_sf32lb58_spi1 == NULL)
    {
      g_sf32lb58_spi1 = sifli_spibus_initialize(SF32LB58_SPI1_PORT);
    }

  return g_sf32lb58_spi1;
}

#ifdef CONFIG_SPI_DRIVER
static int sf32lb58_spi1_register_char(void)
{
  FAR struct spi_dev_s *spi;
  int ret;

  spi = sf32lb58_spi1_getbus();
  if (spi == NULL)
    {
      return -ENODEV;
    }
...
static int sf32lb58_tfcard_mount(void)
{
  ...
  tmpret = mkdir(SF32LB58_TFCARD_MOUNTPOINT, 0755);
  if (tmpret < 0 && errno != EEXIST)
    {
      serr("WARN: mkdir %s failed: %d\n", SF32LB58_TFCARD_MOUNTPOINT, errno);
      return -errno;
    }

  for (i = 0; i < sizeof(devpaths) / sizeof(devpaths[0]); i++)
    {
      ret = nx_mount(devpaths[i], SF32LB58_TFCARD_MOUNTPOINT, "vfat", 0, NULL);
      if (ret >= 0 || ret == -EBUSY)
        {
          syslog(LOG_INFO, "INFO: TF mounted from %s to %s\n",
                 devpaths[i], SF32LB58_TFCARD_MOUNTPOINT);
          return OK;
        }
    }
...
static int sf32lb58_tfcard_initialize(void)
{
  FAR struct spi_dev_s *spi;
  int ret;
  ...
  if (spi == NULL)
    {
      serr("ERROR: sifli_spibus_initialize(%d) failed\n",
           SF32LB58_TFCARD_SPI_PORT);
      return -ENODEV;
    }

  ret = mmcsd_spislotinitialize(SF32LB58_TFCARD_MINOR,
                                SF32LB58_TFCARD_SLOT,
                                spi);
  if (ret < 0)
    {
...
int sf32lb58_lcd_bringup(void)
{
  ...
  ret = sf32lb58_button_initialize("/dev/buttons");
  if (ret < 0 && ret != -EEXIST)
    {
      syslog(LOG_ERR, "ERROR: sf32lb58_button_initialize() failed: %d\n",
             ret);
      return ret;
    }
#endif
  ...
  tmpret = sf32lb58_spi1_register_char();
  if (tmpret < 0)
    {
...
#if defined(CONFIG_SPI) && defined(CONFIG_MMCSD_SPI) && \
    defined(CONFIG_BSP_USING_SPI1)
  tmpret = sf32lb58_tfcard_initialize();
  if (tmpret < 0)
    {
...
#ifdef CONFIG_UART_BTH4
  tmpret = sf32lb58_bt_initialize();
  if (tmpret < 0 && tmpret != -EEXIST)
    {
      serr("WARN: sf32lb58_bt_initialize failed: %d\n", tmpret);
    }
#endif
