...
/* APS 128p — uses MPI1_DQSDM which may not exist on all chips */
#ifdef MPI1_DQSDM
...
#else
    /* Modes 1/2/4 use pinmux names (MPI1_CLKB, MPI1_DM, MPI1_DQSDM,
     * MPI1_DQS) that may not exist on all chip variants.  Guard each
     * case so the code compiles on sf32lb58 where only mode 3 (OPSRAM)
     * is used. */
